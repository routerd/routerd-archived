...
func (p *parser) Parse() (*File, error) {
	...
	for {
		...
		case internal.COMMENT:
			...
			comment += strings.TrimSpace(lit[1:]) // strip # or ;
			continue
		...
		case internal.NEWLINE:
			if key != nil && !strings.HasSuffix(key.Value, "\\") {
				// stop scanning value
				key.Comment = comment
				key = nil
				comment = ""
		...
		case internal.STRING:
			if section == nil {
				return nil, fmt.Errorf("%s: key started outside of section %q", pos, lit)
			}

			if key == nil {
				// we need to start a new key
				if pos, tok, lit := p.scanner.Scan(); tok != internal.ASSIGN {
					return nil, fmt.Errorf("%s: key not followed by = (ASSIGN), token found: %s %q", pos, tok, lit)
				}

				section.Keys = append(section.Keys, Key{
					Name: strings.TrimSpace(lit),
				})
				key = &section.Keys[len(section.Keys)-1]
				continue
			}

			tlit := strings.TrimSpace(lit)
			if strings.HasSuffix(tlit, "\\") {
				// strip \ and replace by space
				key.Value += tlit[:len(tlit)-1] + " "
				continue
			}

			// no multi-line value
			key.Value += tlit
			key.Value = strings.ReplaceAll(key.Value, "\\", " ")
			continue

		case internal.SECTION:
			// validate section name
			if !strings.HasPrefix(lit, "[") {
				return nil, fmt.Errorf("%s: section needs to start with [, is: %q", pos, lit)
			}
			if !strings.HasSuffix(lit, "]") {
				return nil, fmt.Errorf("%s: section needs to end with ], is: %q", pos, lit)
			}
